...
| App-specific | `App-{component}` | `App-api`, `App-ui`, `App-sdk-check` |
| Testing skill | `App-test-{component}` | `App-test-sdk`, `App-test-api` |
...
## Decision: App-Specific vs Generic

```
Patterns apply to ANY project?     → Generic skill (e.g., pytest, typescript)
Patterns are App-specific?     → App-{name} skill
Generic skill needs App info?  → Add references/ pointing to App docs
```
...
| `license` | Yes | Always `Apache-2.0` for App |
