...
# Current: agents/context/skills/skill-sync/assets/sync.sh
# Root: agents/
# Up 4 levels: sync.sh -> assets -> skill-sync -> skills -> context -> agents
REPO_ROOT="$(dirname "$(dirname "$(dirname "$(dirname "$SCRIPT_DIR")")")")"
SKILLS_DIR="$REPO_ROOT/context/skills"
ROUTES_FILE="$REPO_ROOT/context/routes.json"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --scope      Only sync specific scope (context name)"
            exit 0
            ;;
        *)
...
if [ ! -f "$ROUTES_FILE" ]; then
    echo -e "${RED}Error: routes.json not found at $ROUTES_FILE${NC}"
    exit 1
fi

# Load Routes
declare -A ROUTES
# Read JSON strictly with jq-like logic (simple grep/awk for standard json structure)
# Assumes flat key-value pairs: "key": "value"
while IFS="=" read -r key value; do
    ROUTES["$key"]="$value"
done < <(grep -o '"[^"]*": *"[^"]*"' "$ROUTES_FILE" | sed 's/"//g; s/: /=/')
...
echo -e "${BLUE}Skill Sync - Centralized Routing${NC}"
echo "=================================="
echo ""
...
# Generate Monolithic Auto-invoke Section
full_section="### Auto-invoke Skills
...
for scope in "${scopes_sorted[@]}"; do
    path="${ROUTES[$scope]}"
    if [ -z "$path" ]; then
        echo -e "${YELLOW}Warning: Unknown scope '$scope' (not in routes.json)${NC}"
        path="unknown"
    fi

    echo -e "${BLUE}Processing Context: $scope ($path)${NC}"
...
if $DRY_RUN; then
    echo -e "${YELLOW}[DRY RUN] Would update $AGENTS_FILE with:${NC}"
    echo "$full_section"
else
    ...
    if grep -q "### Auto-invoke Skills" "$AGENTS_FILE"; then
        ...
        # Insert if missing (simplified)
         cat "$section_file" >> "$AGENTS_FILE"
         echo -e "${GREEN}  ✓ Appended to Root AGENTS.md${NC}"
    fi
    rm -f "$section_file"
fi
