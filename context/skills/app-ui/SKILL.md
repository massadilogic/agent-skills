...
name: app-ui
description: >
  {Application} UI-specific patterns. For generic patterns, see: typescript, react-19, nextjs-15, tailwind-4.
  Trigger: When working inside ui/ on {Application}-specific conventions (shadcn vs HeroUI legacy, folder placement, actions/adapters, shared types/hooks/lib).
license: Apache-2.0
metadata:
  author: {author}
  version: "1.0"
  scope: [root, ui]
  auto_invoke:
    - "Creating/modifying {Application} UI components"
    - "Working on {Application} UI structure (actions/adapters/types/hooks)"
...
- `playwright` - E2E testing (see also `app-test-ui`)
